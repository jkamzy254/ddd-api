...
async def get_scores(excel):
    ...
    if exam_rec and 'ID' in exam_rec:
        ...
        print("No eligible exam record found in ExamListTable.")
...
def get_db_recs(exam_id):
    with connection.cursor() as cursor:
        sql = "SELECT * FROM ExamNotPassedFunction(%s) Order By ODID, GID, Pos, ID"
...
def get_excel(exam_rec, recs):
    ...
    exam_title = f"AUS church(Melbourne) Sealing verification exam ({exam_name}) Result"
    ...
    filename = f"SCJ_{scj_yr}_Sealing_verification_{cleaned_text}_Score_ReportEN.xlsx"
...
def get_chart(exam_rec):
    ...
    with connection.cursor() as cursor:
        sql = "SELECT * FROM ExamPassRateFunction(%s) Order By ODID"
        cursor.execute(sql, [exam_id])
        recs = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
