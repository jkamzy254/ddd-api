...
def bbstatus(g):
    ...
    dNP  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'New P'", conn)
    dOP  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'Old P'", conn)
    dAB  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'ABB'", conn)
    dIM  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'IBB ME'", conn)
    dIF  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'IBB FA'", conn)
    dFP  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'Fallen P'", conn)
    dAC  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'ABB CCT'", conn)
    dIC  = pd.read_sql(f"SELECT * FROM ScottNewStatus('{g}') WHERE NewStatus = 'IBB CCT'", conn)
    dPts = pd.read_sql(f"SELECT NewStatus, SUM(Points)Pts FROM ScottNewStatus('{g}') GROUP BY NewStatus", conn)
    dNP.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dOP.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dAB.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dIM.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dIF.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dFP.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dAC.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
    dIC.columns = ['UID','FruitName','L1N','L1G','L2N','L2G','BBTN','BBTG','NewStatus','LastClassDate','LastTopic','NextClassDate','Points']
