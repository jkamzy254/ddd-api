...
class GetBBTDataViewSet(APIView):
    def get(self, request):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT UID,ID,Dept,Grp,BBT,P,ACT,INACT,CCT,Total,FE FROM BBTPerformanceSummarizedView ORDER BY GID, Pos, BBT")
                result = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
...
class GetCurrentCTDataViewSet(APIView):
    def get(self, request):
        try:
            with connection.cursor() as cursor:
                cursor.execute("EXEC spBBGetCurrentCTData")
                result = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
...
class GetAllActiveCTDataViewSet(APIView):
    def get(self, request):
        try:
            with connection.cursor() as cursor:
                cursor.execute("EXEC spBBGetAllActiveCTData")
                result = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
...
class GetBBViewSet(APIView):
    def get(self, request):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM BBCurrentList('%','%')")
...
class CheckBBTTransferViewSet(APIView):
    def get(self, request):
        bbt = request.GET.get('BBT')
        fishnum = request.GET.get('FishNum')
        try:
            with connection.cursor() as cursor:
                cursor.execute(f"EXEC spBBCheckBBTTransferDets @BBTID = {bbt}, @FishNum = {fishnum}")
                result = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
...
class GetDenomEthnicViewSet(APIView):
    def get(self, request):
        try:
            with connection.cursor() as cursor:
