...
class FMPRankingGetMembersViewSet(APIView):
    def get(self, request):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("EXEC spFMPRankingGetMembers {0}".format(token['UID']))
...
class FMPGetActiveMembersViewSet(APIView):
    def get(self, request):
        ...
        try:
            ...
            # user = Memberdata.objects.filter(id = payload['ID']).first()
            with connection.cursor() as cursor:
                cursor.execute("EXEC spFMPGetActiveMembers {0}".format(token['UID']))
                recs = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
        except Exception as e:
            # Handle exceptions here, e.g., logging or returning an error response
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class FMPRecordsGroupSummaryViewSet(APIView):
    def get(self, request):
        ...
        try:
            ...
            # user = Memberdata.objects.filter(id = payload['ID']).first()
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM FMPRecordsGroupSummaryView")
                recs = [dict(zip([column[0] for column in cursor.description], record)) for record in cursor.fetchall()]
        except Exception as e:
            # Handle exceptions here, e.g., logging or returning an error response
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
