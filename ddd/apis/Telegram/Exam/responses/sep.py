...
async def get_scores(excel):
    ...
    if exam_rec and 'ID' in exam_rec:
        ...
        print("No eligible exam record found in Sept ExamListTable.")
...
def get_excel(exam_rec, recs):
    ...
    wb = openpyxl.load_workbook('apis/Telegram/Exam/assets/Sep_Exam_Template.xlsx')
    ws = wb.active # Or specify the sheet name
    
    exam_name = exam_rec.get('ExamName')
    
    exam_title = f"AUS church(Melbourne) September Exam ({exam_name}) Result"
